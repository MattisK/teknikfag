...
public class ImmunoTowerScript : MonoBehaviour
{
    public float dmg;
    public float range;
    ...
    ImmunoBulletScript bulletScript;
    ...
    {   
        ...
        placeable = false;
        transform.rotation= Quaternion.Euler(0,0,90); //rotates the tower so it faces the right way
        ...
        StartWaveBtn = mainScript.StartWaveBtn;
    }
    ...
    void Update()
    {
        if(placed == false)
        {
            ...
            if(timePassed >= fireRate && StartWaveBtn.activeSelf == false) //checks if enough time has passed to fire again
            {   
                    timePassed =0; //resets the shooting timer
                    switch (curBullet) 
                    {
                        case 1: //switches between the different bullet types after each shoot
                            tempBullet = Instantiate(bulletOne);
                            bulletScript = tempBullet.GetComponent<ImmunoBulletScript>();
                            bulletScript.motherObject = this.gameObject;
                            curBullet =2;
                            break;
                        case 2:
                            tempBullet = Instantiate(bulletTwo);
                            bulletScript = tempBullet.GetComponent<ImmunoBulletScript>();
                            bulletScript.motherObject = this.gameObject;
                            curBullet =3;
                            break;
                        case 3:
                            tempBullet = Instantiate(bulletThree);
                            bulletScript = tempBullet.GetComponent<ImmunoBulletScript>();
                            bulletScript.motherObject = this.gameObject;
                            curBullet =1;
                            break;
                        default://if the current bullet is invalid it will default to the first bullet type
                            tempBullet = Instantiate(bulletThree);
                            bulletScript = tempBullet.GetComponent<ImmunoBulletScript>();
                            bulletScript.motherObject = this.gameObject;
                            curBullet =1;
                            print("immuno tower is bad fix please");
                            break;
                    }
                    tempBullet.transform.rotation = this.transform.rotation; //makes it go the right direction
                    tempBullet.transform.position = this.transform.position+transform.right * 0.5f; //makes sure it appears in front of the tower
                    tempBullet.GetComponent<ImmunoBulletScript>().dmg = dmg; //sets the damage of the bullet
            }

        }   
        curClosePlace = null;//clears the current closet placement spot
    }

    void OnTriggerStay2D(Collider2D collider) //placement logic
    {
        if(collider.gameObject.tag == "towerIn" && placed == false) //checks if the tower is over a valid placement spot
        {
