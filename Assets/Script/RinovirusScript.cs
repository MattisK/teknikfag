...
public class RinovirusScript : MonoBehaviour
{
    public float health;
    public float speed;
    public float damage;
    ...
    {   
        ...
        health = 5; //how resitant is it (ish)
        speed = 3; //incubation time
        damage = 1; // how sick do you get
        resistances[0]= "antiBiotics.100"; //resists antiBiotics completely since its a virus 
        resistances[1]= "cold.66"; //resists cold based on research by 66%
        resistances[2]= "vaccine.100"; //resists vaccines completely since its a virus
        resistances[3]= "immunity.80"; //resists immunity based on research (to many types of rinovirus to be immune to all of them) by 80%
        resistances[4]= "sprit.20"; //resists sprit based on research  by 20%
        resistanceCount = 5; //how many resistances it has
        weaknesses[0]= "immuneSystem.2"; //takes duble damage from immune system since it needed a weakness and its the main way of fighting of rinovirus
        weaknesses[1]= "warm.3"; //takes 3x damage from warm based on research
      ...
      if(DeathTimer == -10){ //check if the enemy is currently dieing or not 
        ...
        distance -= speed*Time.deltaTime; //counts down distance wtih the speed of the enemy
            ...
            if(DeathTimer > 0.28f){
                ...
                SpriteHolder.GetComponent<Transform>().localScale = new Vector3(0.2f,0.2f,1f); //changes size to accomodate the new sprite
            }
    ...
    public void Die(){ //function kill the enemy and reward the appropriate amount of money
        mainScript.enemies.Remove(gameObject); //removes the enemy from the enemies list
        DeathTimer= 0.3f;
        this.GetComponent<BoxCollider2D>().enabled = false;
        mainScript.money += 5; //gives the player money for killing the enemy
    }
}
